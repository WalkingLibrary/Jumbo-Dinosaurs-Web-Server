...
sudo systemctl stop jumbodinosaurswebserver.service
...
mv build/libs/jumbodinosaurswebserver.jar ./
if [ $? -ne 0 ]; then
...
if [ ! -f "jumbodinosaurswebserver.jar" ]; then
...
sudo systemctl start jumbodinosaurswebserver.service
if [ $? -ne 0 ]; then
